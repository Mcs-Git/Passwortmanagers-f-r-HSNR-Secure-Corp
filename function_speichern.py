...
def password_encryption(entered_password):
    if not os.path.exists("passwords_db.txt"):
        key = os.urandom(256)
        kr.set_password("Secret","Key",f"{key}")
        print(key)
    generated_key = kr.get_password("Secret", "Key")
    print("Password", generated_key)
    h = hmac.HMAC(b"ssiojdow", hashes.SHA256())
    ...
    print("Signature3: ", password_hashed)
...
if not os.path.exists("passwords_db.txt"):
    with open("passwords_db.txt", "w"):
        pass

with open("passwords_db.txt", "a") as f:
    f.write(f"{user_obj}\n")
